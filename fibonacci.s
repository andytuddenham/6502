...
irq:
  ...
  phy                   ; save y register
irq_start:
  lda IFR               ; get interrupt flags
  bpl irq_noirq         ; skip if no interrupt (should not occurr)
  and IER               ; zero out any interrupts we have not enabled
  asl                   ; shift timer1 into bit 7 (not currently enabled)
  asl                   ; shift timer2 into bit 7 (not currently enabled)
  asl                   ; shift cb1 into bit 7  (not currently enabled)
  asl                   ; shift cb2 into bit 7  (not currently enabled)
  asl                   ; shift shift reg into bit 7 (not currently enabled)
  asl                   ; shift ca1 into bit 7
  bmi irq_ca1           ; handle ca1 interrupt
  asl                   ; shift ca2 into bit 7
  bmi irq_ca2           ; handle ca2 interrupt
  jmp irq_exit
irq_ca1:
; TODO
; read top 4 bits of PORTA to determine which button was pressed
; End TODO
  lda #%11000000        ; set display address to line 2 column 1
  jsr lcd_instruction
; set address of left_message into pm_textAddress then call print_message
  lda #(left_message&$00ff)
  sta pm_textAddress
  lda #(left_message>>8)
  sta pm_textAddress + 1
  jsr print_message
...
irq_ca2:
  lda #%11000000        ; set display address to line 2 column 1
  jsr lcd_instruction
; set address of enter_messsage into pm_textAddress then call print_message
  lda #(enter_message&$00ff)
  sta pm_textAddress
  lda #(enter_message>>8)
  sta pm_textAddress + 1
  jsr print_message
...
irq_noirq:
  ...
  lda #%11000000        ; set display address to line 2 column 1
  jsr lcd_instruction
; set address of noirq_message into pm_textAddress then call print_message
  lda #(noirq_message&$00ff)
  sta pm_textAddress
  lda #(noirq_message>>8)
  sta pm_textAddress + 1
  jsr print_message
...
left_message:   .asciiz "Left"
right_message:  .asciiz "Right"
up_message:     .asciiz "Up"
down_message:   .asciiz "Down"
enter_message   .asciiz "Enter"
noirq_message   .asciiz "NoIRQ"
